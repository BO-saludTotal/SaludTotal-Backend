...
import {
  Controller, Post, Body, Param, UsePipes, ValidationPipe, Req,
  HttpException, HttpStatus, UseGuards, ParseUUIDPipe, Get, 
  ForbiddenException
} from '@nestjs/common';
...
export class MedicalHistoryController {
  ...
  async createMedicalEntry(
    ...
    try {
      ...
      console.error(`Error en POST /patients/${patientId}/medical-history/entries:`, error);
  ...
  async getPatientHistoryEntries(
    ...
    if (authenticatedUser.roles.includes(AllowedRoles.Paciente as string) && authenticatedUser.userId !== patientIdFromUrl) {
      throw new ForbiddenException('Los pacientes solo pueden acceder a su propio historial clínico.');
    }

  
    try {
      const entries = await this.medicalHistoryService.getPatientHistoryEntries(patientIdFromUrl);
      if (!entries || entries.length === 0) {
      
        return {
            statusCode: HttpStatus.OK,
            message: 'No se encontraron entradas en el historial para este paciente.',
            data: []
        };
      }
      return {
      ...
      console.error(`Error en GET /patients/${patientIdFromUrl}/medical-history/entries:`, error);
      throw new HttpException(
