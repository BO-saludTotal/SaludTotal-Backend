...
export class GovernmentStaffDetail extends BaseEntity {
    ...
    @Column({ name: 'NombreInstitucionGubernamental', type: 'varchar', length: 255, nullable: false })
    governmentalInstitutionName: string;

    @Column({ name: 'CargoEnInstitucion', type: 'varchar', length: 150, nullable: false })
    positionInInstitution: string;

    @OneToOne(() => User, user => user.governmentStaffDetail, { onDelete: 'CASCADE', onUpdate: 'CASCADE' })
