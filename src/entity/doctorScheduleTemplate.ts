...
export class DoctorScheduleTemplate extends BaseEntity {
    ...
    @ManyToOne(() => User, user => user.scheduleTemplatesAsDoctor, { onDelete: 'CASCADE', onUpdate: 'CASCADE' })
    @JoinColumn({ name: 'MedicoUsuarioID_Ref', referencedColumnName: 'id' })
    doctorUser: User; 

    @ManyToOne(() => HealthEntity, healthEntity => healthEntity.scheduleTemplates, { onDelete: 'CASCADE', onUpdate: 'CASCADE' })
    @JoinColumn({ name: 'EntidadSaludID_Ref', referencedColumnName: 'id' })
    healthEntity: HealthEntity;

    @ManyToOne(() => PhysicalAttentionSpace, space => space.scheduleTemplates, { onDelete: 'SET NULL', onUpdate: 'CASCADE', nullable: true })
