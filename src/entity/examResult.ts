...
export class ExamResult extends BaseEntity {
    ...
    @Column({ name: 'NombreExamenGeneral', type: 'varchar', length: 255, nullable: false })
    generalExamName: string;

    @Column({ name: 'FechaEmisionResultado', type: 'datetime', nullable: true })
    resultIssueDate?: Date | null;

    @ManyToOne(() => ClinicalRecordEntry, entry => entry.examResults, { onDelete: 'CASCADE', onUpdate: 'CASCADE' })
