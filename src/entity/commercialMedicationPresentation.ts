...
export class CommercialMedicationPresentation extends BaseEntity {
    ...
    @Column({ name: 'NombreComercial', type: 'varchar', length: 255, nullable: false })
    commercialName: string;

    @Column({ name: 'LaboratorioFabricante', type: 'varchar', length: 255, nullable: true })
    manufacturerLaboratory?: string | null;

    @ManyToOne(() => GeneralMedication, gm => gm.commercialPresentations, { onDelete: 'CASCADE', onUpdate: 'CASCADE' })
    @JoinColumn({ name: 'MedicamentoGeneralID_Ref', referencedColumnName: 'id' })
    generalMedication: GeneralMedication;

    @OneToMany(() => PrescriptionMedicationDetail, detail => detail.medicationPresentation)
