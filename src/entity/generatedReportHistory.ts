...
export class GeneratedReportHistory extends BaseEntity {
    ...
    @Column({ name: 'UsuarioID_Solicitante_Ref', type: 'varchar', length: 36, nullable: true })
    ...
    parametersUsed?: any | null; 

    @Column({ name: 'EnlaceAlmacenamientoReporte', type: 'text', nullable: true })
    reportStorageLink?: string | null;

    @ManyToOne(() => ReportType, type => type.generatedReports, { onDelete: 'RESTRICT', onUpdate: 'CASCADE' })
    @JoinColumn({ name: 'TipoReporteID_Ref', referencedColumnName: 'id' })
    reportType: ReportType;

    @ManyToOne(() => User, user => user.generatedReports, { onDelete: 'SET NULL', onUpdate: 'CASCADE', nullable: true })
