...
export class ExamResultDetail extends BaseEntity {
    ...
    @Column({ name: 'ValorObtenido', type: 'varchar', length: 255, nullable: false })
    obtainedValue: string;

    @ManyToOne(() => ExamResult, er => er.parameterDetails, { onDelete: 'CASCADE', onUpdate: 'CASCADE' })
    @JoinColumn({ name: 'ResultadoExamenID_Ref', referencedColumnName: 'id' })
    examResult: ExamResult;

    @ManyToOne(() => ExamParameter, ep => ep.examResultDetails, { onDelete: 'RESTRICT', onUpdate: 'CASCADE' })
