...
export class AppointmentChangeHistory extends BaseEntity {
    ...
    @Column({ name: 'UsuarioID_RealizaCambio_Ref', type: 'varchar', length: 36, nullable: true })
    changedByUserId?: string | null;

    @Column({ name: 'EstadoAnterior', type: 'varchar', length: 50, nullable: true })
    ...
    @ManyToOne(() => MedicalAppointment, appointment => appointment.changeHistory, { onDelete: 'CASCADE', onUpdate: 'CASCADE' })
    @JoinColumn({ name: 'CitaID_Ref', referencedColumnName: 'id' })
    appointment: MedicalAppointment;

    @ManyToOne(() => User, { onDelete: 'SET NULL', onUpdate: 'CASCADE', nullable: true }) 
    @JoinColumn({ name: 'UsuarioID_RealizaCambio_Ref', referencedColumnName: 'id' })
